...
namespace BrickGame
{
    public class Ball
    {
        ...
        BrickData brick;
        public void SetBrick(BrickData brick) { this.brick = brick; }
        ...
        public int Collision(int x, int y)
        {
            ...
            foreach (var brick in brick.Bricks)
            {
                ...
                {
                    // 공의 이동 방향 고려
                    bool hitTop = (m_tBall.nY < brick.Y);  // 공이 벽돌 위에서 내려옴
                    bool hitBottom = (m_tBall.nY > brick.Y);  // 공이 벽돌 아래에서 올라옴
                    bool hitLeft = (m_tBall.nX < brick.X);  // 공이 왼쪽에서 이동
                    bool hitRight = (m_tBall.nX > brick.X); // 공이 오른쪽에서 이동

                    if (hitTop || hitBottom)
                    {
                        // Y축 반전
                        if (m_tBall.nDirect == 0) m_tBall.nDirect = 3;
                        else if (m_tBall.nDirect == 3) m_tBall.nDirect = 0;
                        else if (m_tBall.nDirect == 1) m_tBall.nDirect = 2;
                        else if (m_tBall.nDirect == 2) m_tBall.nDirect = 1;
                        else if (m_tBall.nDirect == 4) m_tBall.nDirect = 5;
                        else if (m_tBall.nDirect == 5) m_tBall.nDirect = 4;
                    }

                    if (hitLeft || hitRight)
                    {
                        // X축 반전
                        if (m_tBall.nDirect == 1) m_tBall.nDirect = 5;
                        else if (m_tBall.nDirect == 5) m_tBall.nDirect = 1;
                        else if (m_tBall.nDirect == 2) m_tBall.nDirect = 4;
                        else if (m_tBall.nDirect == 4) m_tBall.nDirect = 2;
                    }
        ...
        public void Initialize()
        {
            ...
            m_tBall.nDirect = 1;
            m_tBall.nX = Console.WindowWidth/2;
            m_tBall.nY = Console.WindowHeight/2;
