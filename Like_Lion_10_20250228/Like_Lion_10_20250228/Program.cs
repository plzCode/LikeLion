...
namespace Like_Lion_10_20250228
{
    //미사일 클래스
    public class BULLET
    {
    ...
    public class Item
    {
        ...
        public void ItemDraw()
        {
            ...
            if (itemY <= 1)
                itemY++;
            if(itemX >= Console.WindowWidth)
                itemX--;
            if (itemY >= Console.WindowHeight)
                itemY--;*/
            if (itemX <= 0 || itemY <= 0 || itemX >= Console.WindowWidth || itemY >= Console.WindowHeight) return;
        ...
        public void ItemMove()
        {
            ...
            if (itemX <= 0)
            {
                vec = rand.Next(2, 4); //34
            }
            if(itemY <= 0)
            {
                vec = rand.Next(1, 3); //12
            }
            if(itemX >= Console.WindowWidth)
            {
                vec = rand.Next(0, 2); //01
            }
            if(itemY >= Console.WindowHeight)
            {
                vec = rand.Next(0, 2); //01
